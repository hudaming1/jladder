...
public class NettyHttpProxyEncShakeHanlder extends ChannelInboundHandlerAdapter {
	...
	private static final Logger logger = LoggerFactory.getLogger(NettyHttpProxyEncShakeHanlder.class);
    ...
    public void channelRead(ChannelHandlerContext outsideProxyCtx, Object msg) throws Exception {
        ...
        if (byteBuf.readInt() != Constant.MAGIC_NUMBER || byteBuf.readInt() != NettyProxyBuildSuccessMessage.SUCCESS) {
        	logger.warn("outside-server connect " + req.getHost() + ":" + req.getPort() + " failure. "); // 告知断开连接
        ...
        logger.info("outside-server connect server [{}:{}] successfully", req.getHost(), req.getPort());
        ...
        if (req.isHttps()) { 
            ...
            logger.info("outside-server build-https-connection [{}:{}] successfully", req.getHost(), req.getPort());
        	return ;
        } 
        ...
        logger.info("outside-server build-http-connection [{}:{}] successfully", req.getHost(), req.getPort());
    }
}