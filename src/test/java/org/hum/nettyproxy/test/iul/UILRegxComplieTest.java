...
public class UILRegxComplieTest {
	...
	public void test1() {
		...
		assert(regx1.getAction().getType() == InterceptorTypeEnum.Request);
		assert(regx1.getAction().getField() == InterceptorFieldEnum.Header);
		assert(regx1.getAction().getValue().equals("localhost:8080"));
		assert(regx1.getMatch().get(0).getType() == InterceptorTypeEnum.Request);
		assert(regx1.getMatch().get(0).getField() == InterceptorFieldEnum.Header);
